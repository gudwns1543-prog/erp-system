...
export default function DashboardPage() {
  ...
  // 출근 자체를 막는 휴무성 기록만 제한합니다. 출장/외근은 출근·복귀와 병행될 수 있어 제외합니다.
  const LEAVE_NOTES = ['연차','반차(오전)','반차(오후)','반반차','병가','공가','특별휴가']
  const leaveSession = sessions.find((s:any) => LEAVE_NOTES.includes(s.note))
  const activeSession = !leaveSession ? [...sessions].reverse().find((s:any) => s.check_in && !s.check_out) : null
  // 오늘 마지막 완료 세션
  ...
  async function handleCheckIn() {
    ...
    // DB의 unique(user_id, work_date, seq) 제약 때문에 복귀출근은 반드시 다음 seq로 저장해야 합니다.
    ...
    if (loadErr) {
      setAlert('출근 전 세션 확인 실패: ' + loadErr.message)
