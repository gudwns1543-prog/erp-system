...
export default function PayrollPage() {
  ...
  async function saveOverrides() {
    ...
    for (const [k, v] of Object.entries(payOverrides)) if (v && v !== 0) items[k] = v
    for (const [k, v] of Object.entries(deductOverrides)) if (v && v !== 0) items[k] = v
              ...
              <span>계약연봉:
                {editingAnnual ? (
                  <span className="inline-flex items-center gap-1 ml-1">
                    <input type="number" className="input text-xs py-0.5 w-32" value={newAnnual}
                      onChange={e=>setNewAnnual(+e.target.value)} />
                    <button onClick={saveAnnual} className="btn-primary text-xs px-2 py-0.5">저장</button>
                    <button onClick={()=>setEditingAnnual(false)} className="text-xs text-gray-400">취소</button>
                  </span>
                ) : (
                  <span className="ml-1 font-semibold text-gray-800">
                    {formatWon(selSalary.annual)}
                    <button onClick={()=>setEditingAnnual(true)} className="text-[10px] text-purple-500 hover:text-purple-700 ml-1">수정</button>
                  </span>
                )}
              </span>
