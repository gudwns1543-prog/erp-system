...
export default function ChatPage() {
  ...
  // 읽음 계산이 진행 중인지 추적 (중복 방지)
  const calcingRef = useRef(false)

  // ── 읽음 계산 (메시지/멤버 리로드 없음) ──────────────────────
  async function calcReceipts(roomId: string, msgs: any[], mems: any[]) {
    if (calcingRef.current) return
    calcingRef.current = true
    try {
      ...
      setReadReceipts(result)
    } finally {
      calcingRef.current = false
    }
  }
  ...
  async function loadRooms(uid: string) {
    ...
    const roomsWithMembers = (data||[]).map(r => {
      const roomMems = (allMems||[]).filter((m:any) => m.room_id === r.id)
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`chat-room-${activeRoomId}`)
      // 새 메시지 → 메시지만 리로드
      .on('postgres_changes', {
        ...
        if (session) {
          await supabase.from('chat_reads').upsert(
            { room_id: activeRoomId, user_id: session.user.id, last_read_at: new Date().toISOString() },
            { onConflict: 'room_id,user_id' }
          )
        }
        const mems = await fetchMembers(activeRoomId)
        await calcReceipts(activeRoomId, msgs, mems)
      })
      // 읽음 변경 → 읽음 계산만 (메시지 리로드 X)
      .on('postgres_changes', {
        event: 'INSERT', schema: 'public', table: 'chat_reads',
        filter: `room_id=eq.${activeRoomId}`
      }, async () => {
        if (activeRoomIdRef.current !== activeRoomId) return
        const [msgs, mems] = await Promise.all([fetchMessages(activeRoomId), fetchMembers(activeRoomId)])
        await calcReceipts(activeRoomId, msgs, mems)
      })
      .on('postgres_changes', {
  ...
  return (
    <div className="p-6 h-[calc(100vh-48px)] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {[...rooms].sort((a,b)=>{
              ...
              return (
                <div key={r.id}
                  onClick={()=>{
                    setActiveRoom(r); setActiveRoomId(r.id)
      ...
      {/* 토스트 알림 */}
      {toast && (
        <div onClick={()=>{ const r=rooms.find(r=>r.name===toast.room); if(r){setActiveRoom(r);setActiveRoomId(r.id);setUnreadCounts(p=>({...p,[r.id]:0}))} setToast(null) }}
